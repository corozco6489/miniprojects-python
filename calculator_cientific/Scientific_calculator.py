...
ventana.title("Scientific Calculator")

ventana.config(bg='black')

ventana.resizable(0, 0)
ventana.geometry("480x528+450+90")

calc = Frame(ventana, bg='black')
calc.grid()
...
def Standard():
    ...
    ventana.geometry("480x528+0+0")


def Scientific():
    ventana.resizable(width=False, height=False)
    ventana.geometry("980x528+0+0")
...
txtDisplay = Entry(calc, font=('Helvetica', 20, 'bold'),
                   bg='#000008', fg='white', bd=30, width=28, justify=RIGHT)
...
btnClear = Button(calc, text=chr(67), width=6, height=2, bg='powder blue', font=(
    'Helvetica', 20, 'bold'), bd=4, command=added_value.Clear_Entry)
...
btnsq = Button(calc, text='\u221A', width=6, height=2,
               bg='yellow', font=('Helvetica', 20, 'bold'), bd=4)
btnsq.grid(row=1, column=2, pady=1)


btnAdd = Button(calc, text='+', width=6, height=2, bg='yellow', font=('Helvetica',
                20, 'bold'), bd=4, command=lambda: added_value.operation('add'))
btnAdd.grid(row=1, column=3, pady=1)


btnSub = Button(calc, text='-', width=6, height=2, bg='yellow', font=('Helvetica',
                20, 'bold'), bd=4, command=lambda: added_value.operation('sub'))
btnSub.grid(row=4, column=3, pady=1)

btnMul = Button(calc, text='x', width=6, height=2, bg='yellow', font=(
    'Helvetica', 20, 'bold'), bd=4, command=lambda: added_value.operation('multi'))
btnMul.grid(row=2, column=3, pady=1)

btnDiv = Button(calc, text='/', width=6, height=2, font=('Helvetica', 20, 'bold'),
                bd=4, bg='yellow', command=lambda: added_value.operation('divide'))
btnDiv.grid(row=3, column=3, pady=1)

btnZero = Button(calc, text='0', width=6, height=2, bg='black', fg='white', font=(
    'Helvetica', 20, 'bold'), bd=4, command=lambda: added_value.numberEnter(0))
btnZero.grid(row=5, column=0, pady=1)

btnDot = Button(calc, text='.', width=6, height=2, font=('Helvetica', 20,
                'bold'), bd=4, bg='yellow', command=lambda: added_value.numberEnter('.'))
btnDot.grid(row=5, column=1, pady=1)

btnPM = Button(calc, text=chr(177), font=('Helvetica', 20,
               'bold'), bg='yellow', height=2, width=6, bd=4)
btnPM.grid(row=5, column=2, pady=1)

btnEquals = Button(calc, text='=', font=('Helvetica', 20, 'bold'),
                   height=2, width=6, bg='yellow', bd=4, command=added_value.sum_of_total)
...
btnMod = Button(calc, text="Mod", width=6,
                height=2, bg='yellow',
                font=('Helvetica', 20, 'bold'),
                bd=4,command=added_value.mathPM).grid(row=3, column=6, pady=1)
...
btnexpm = Button(calc, text="expm1", width=6,
                 height=2, bg='yellow',
                 font=('Helvetica', 20, 'bold'),
...
btnasndh = Button(calc, text="asinh", width=6,
                  height=2, bg='yellow',
                  font=('Helvetica', 20, 'bold'),
